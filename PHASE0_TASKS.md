...
| P0-H7 | Tutor avatars — ported Orb + Free (code-drawn) renderers + animation engine from the Animation project (pure Compose, no Rive/TTS) | ✅ | `avatar/` (core, animation, renderer, ui/theme) + `EduTutorAvatar.kt`; Orb mascot in `TopBarChips`, celebrating Orb in `RewardOverlay` |
| P0-H8 | Avatar tab + studio — new bottom-nav Avatar tab with full playground (character, mood, outfit ×4, neck/hair/hairColor/glasses/frame ×3, eye-line & cheeks toggles, mood ×6, gesture ×6, spin); minimized Free avatar bubble on Home | ✅ | `EduBottomNavItem.Avatar`, `screens/AvatarStudioScreen.kt`, `HomeTutorBubble` in `HomeRails.kt`; `EduTutorAvatar` extended to forward all Free options |
| P0-H9 | Save the tutor — SharedPreferences-persisted look; Save button in studio; Home mascot, top-bar avatar, and reward orb all reflect the saved config | ✅ | `avatar/TutorConfig.kt` (store + `SavedTutorAvatar`); Save wired in `AvatarStudioScreen`; `HomeRails`/`TopBarChips`/`RewardOverlay` use `SavedTutorAvatar` |
| P0-H10 | Retention loop — weekly-rotating premium avatars, unlock via simulated 2× rewarded video, persisted unlocks, Use + Share (native intent) | ✅ | `avatar/AvatarPresets.kt` (pool + weekly rotation + unlock store), `avatar/AdReward.kt` (`AdRewardOverlay`), `avatar/AvatarShare.kt`; "This week's avatars" + Share wired in `AvatarStudioScreen`. Simulated ads — swap for AdMob at launch |
